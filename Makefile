...
EXCLUDE_SRC_FROM_LIB+=$(foreach file, $(SRCDIR)/opcontrol $(SRCDIR)/autonomous, $(SRCDIR)/$(LIBNAME)/Robot.cpp $(SRCDIR)/$(LIBNAME)/commands/ExampleCommand.cpp $(SRCDIR)/$(LIBNAME)/ExampleSubsystem.cpp , $(foreach cext,$(CEXTS),$(file).$(cext)) $(foreach cxxext,$(CXXEXTS),$(file).$(cxxext)))
